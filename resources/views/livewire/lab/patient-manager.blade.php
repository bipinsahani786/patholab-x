<div>
    ...
    <div class="main-content">
        ...
        @if (session()->has('message'))
            <div
                class="alert alert-success border-0 shadow-sm rounded-3 d-flex align-items-center py-3 alert-dismissible fade show">
        ...
        @if (session()->has('error'))
            <div
                class="alert alert-danger border-0 shadow-sm rounded-3 d-flex align-items-center py-3 alert-dismissible fade show">
                                ...
                                <tr wire:key="patient-{{ $patient->id }}" class="border-bottom border-light">
                                    <td class="ps-4 py-3">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="bg-soft-primary text-primary rounded-circle d-flex align-items-center justify-content-center fw-bold fs-5 shadow-sm"
                                            ...
                                            <div>
                                                ...
                                                <div class="fs-12 mt-1">
                                                    <span
                                                        class="badge bg-soft-info text-info border border-info border-opacity-25">{{ $patient->formatted_id }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="fs-13 text-dark fw-medium">
                                            ...
                                            {{ $patient->patientProfile->age_type ?? 'Yrs' }},
                                            {{ $patient->patientProfile->gender ?? 'N/A' }}
                                        </div>
                                        @if(!empty($patient->patientProfile->blood_group))
                                            <div class="fs-12 text-danger mt-1 fw-bold"><i
                                                    class="feather-droplet me-1"></i>Blood:
                                                {{ $patient->patientProfile->blood_group }}</div>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="fs-13 text-dark fw-medium mb-1"><i
                                                class="feather-phone me-1 text-muted"></i>{{ $patient->phone }}</div>
                                        <div class="fs-12 text-muted text-truncate" style="max-width: 200px;">
                                            <i
                                                class="feather-map-pin me-1"></i>{{ $patient->patientProfile->address ?? 'No address provided' }}
                                        </div>
                                    </td>
                                    <td class="text-center pe-4">
                                        <div class="d-flex justify-content-center gap-2">
                                            @if($patient->activeMembership)
                                                <a href="{{ route('lab.membership.card.print', $patient->activeMembership->id) }}"
                                                    target="_blank"
                                                    class="btn btn-sm btn-light border text-success shadow-sm rounded align-center-btn transition-all hover-success"
                                            ...
                                            @if(auth()->user()->can('edit patients') || auth()->user()->collection_center_id)
                                                <button wire:click="edit({{ $patient->id }})"
                                                    class="btn btn-sm btn-light border text-primary shadow-sm rounded align-center-btn transition-all hover-primary"
                                                    title="Edit Profile">
                                            ...
                                            @can('delete patients')
                                                <button wire:click="delete({{ $patient->id }})"
                                                    wire:confirm="Warning: This will delete the patient and all associated data. Continue?"
                                                    class="btn btn-sm btn-light border text-danger shadow-sm rounded align-center-btn transition-all hover-danger"
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="text-muted mb-3"><i class="feather-users"
                                                style="font-size: 3.5rem; opacity: 0.5;"></i></div>
                ...
                <div class="modal-content border-0 shadow-lg rounded-4">
                    ...
                    <form wire:submit.prevent="store">
                        <div class="modal-body p-4 bg-white">
                            <div class="row g-4">
                                <div class="col-12">
                                    <h6 class="fw-bold text-primary mb-0 border-bottom pb-2">Basic Information</h6>
                                </div>

                                <div class="col-md-6">
                                    <label class="form-label fs-12 fw-bold text-muted text-uppercase">Full Name *</label>
                                    <input type="text" class="form-control fw-medium text-dark" wire:model="name"
                                ...
                                <div class="col-12 mt-4">
                                    <h6 class="fw-bold text-primary mb-0 border-bottom pb-2">Demographics & Medical</h6>
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    @error('age') <span class="text-danger fs-11 fw-bold d-block">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    @error('blood_group') <span class="text-danger fs-11 fw-bold">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" wire:model="address" rows="2"
                                        placeholder="Enter complete patient address..."></textarea>
                                    @error('address') <span class="text-danger fs-11 fw-bold">{{ $message }}</span>
                                    @enderror
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="modal-footer bg-light border-top p-3 d-flex justify-content-end gap-2">
                            <button type="button" wire:click="closeModal"
                                class="btn btn-light border px-4 fw-medium shadow-sm">Cancel</button>
                            <button type="submit" class="btn btn-primary px-5 fw-bold shadow-sm d-flex align-items-center">
                                <div wire:loading.remove wire:target="store"><i class="feather-save me-2"></i> Save Patient
                                </div>
                                <div wire:loading wire:target="store"><span class="spinner-border spinner-border-sm me-2"
                                        role="status"></span> Saving...</div>
                            </button>
                        </div>
                    </form>
                </div>
